...
class TelegramDriver extends HttpDriver
{
    ...
    public function buildServicePayload($message, $matchingMessage, $additionalParameters = [])
    {
        ...
        if ($message instanceof Question) {
            ...
            if ($message->getAttachment() !== null) {
                ...
                $parameters['filename'] = $attachment->getExtras('filename');
                $parameters['mimetype'] = $attachment->getExtras('mimetype');
                ...
                if ($attachment instanceof Image) {
                    if (strtolower(pathinfo($attachment->getUrl(), PATHINFO_EXTENSION)) === 'gif') {
                        ...
                        $parameters['document'] = $attachment->getUrl() ?? $attachment->getPayload();
                    } else {
                        $this->endpoint = 'sendPhoto';
                        $parameters['photo'] = $attachment->getUrl() ?? $attachment->getPayload();
                    }
                    // If has a title, overwrite the caption
                    if ($attachment->getTitle() !== null) {
                        $parameters['caption'] = $attachment->getTitle() ?? $attachment->getPayload();
                    }
                } elseif ($attachment instanceof Video) {
                    $this->endpoint = 'sendVideo';
                    $parameters['video'] = $attachment->getUrl() ?? $attachment->getPayload();
                } elseif ($attachment instanceof Audio) {
                    $this->endpoint = 'sendAudio';
                    $parameters['audio'] = $attachment->getUrl() ?? $attachment->getPayload();
    ...
    protected function buildDataFiles($boundary, $fields, $files)
    {
        $data = '';
        $eol = "\r\n";
    ...
    protected function uploadBinaryFile($url, $key, $payload) {
        ...
        $postData = $this->buildDataFiles($boundary, $payload, [$key => ['content' => $data, 'filename' => $filename]]);
    ...
    public function sendPayload($payload)
    {
        ...
        $postFields = ['document', 'photo', 'video', 'audio'];
        foreach ($postFields as $field) {
            if (isset($payload[$field]) && substr($payload[$field], 0, 4) !== 'http') {
                return $this->uploadBinaryFile($this->buildApiUrl($this->endpoint), $field, $payload);
            }
        }
